...
static int __privilege_manager_check_privilege_list(const char *api_version, const char *privilege, GList * vaild_privilege_list, int *privilege_level, char **changed_to, char **valid_api_version)
{
	...
	for (l = vaild_privilege_list; l != NULL; l = l->next) {
		...
		if (strcmp(privilege_info_db_row->privilege_name, privilege) == 0) {
			...
			TryReturn(tmp_api_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] tmp_api_version's strdup is failed.");
			...
			TryReturn(tmp_api_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] tmp_api_version's realloc is failed.");
			...
			TryReturn(tmp_expired_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] tmp_expired_version's strdup is failed.");
			...
			TryReturn(tmp_expired_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] tmp_expired_version's realloc is failed.");
			...
			TryReturn(tmp_issued_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] tmp_issued_version's strdup is failed.");
			...
			TryReturn(tmp_issued_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] tmp_issued_version's realloc is failed.");
			...
			if (is_valid_version == 0) {
				...
				*valid_api_version = strdup(privilege_info_db_row->expired_version);
				TryReturn(valid_api_version != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
						  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] valid_api_version's strdup is failed.");
				...
				if (privilege_info_db_row->changed_to != NULL && strcmp(privilege_info_db_row->changed_to, "") != 0) {
					...
					*changed_to = strdup(privilege_info_db_row->changed_to);
					TryReturn(changed_to != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
							  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] changed_to's strdup is failed.");
				}
...
int privilege_manager_verify_privilege(const char *api_version, privilege_manager_package_type_e package_type, GList * privilege_list, privilege_manager_visibility_e visibility, char **error_message)
{
	...
	for (l = privilege_list; l != NULL; l = l->next) {
		...
		if (ret == PRVMGR_ERR_NO_EXIST_PRIVILEGE) {
			...
			if (noexist_message == NULL) {
				...
				TryReturn(noexist_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
						  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] noexist_message's strdup is failed.");
			}
			size_t new_size = snprintf(0, 0, "%s%s", noexist_message, message) + 1;
			noexist_message = realloc(noexist_message, new_size * sizeof(char));
			TryReturn(noexist_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] noexist_message's realloc is failed.");
			...
			if (deprecated_message == NULL) {
				...
				TryReturn(deprecated_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
						  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] deprecated_message's strdup is failed.");
			}
			...
			TryReturn(deprecated_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] deprecated_message's realloc is failed.");
			...
			if ((visibility & PRVMGR_PACKAGE_VISIBILITY_PUBLIC) == PRVMGR_PACKAGE_VISIBILITY_PUBLIC) {
				if (privilege_level_id == PRIVILEGE_DB_MANAGER_PRIVILEGE_LEVEL_PARTNER || privilege_level_id == PRIVILEGE_DB_MANAGER_PRIVILEGE_LEVEL_PLATFORM) {
					...
					if (mismatched_message == NULL) {
						...
						TryReturn(mismatched_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
								  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] mismatched_message's strdup is failed.");
					}
					...
					TryReturn(mismatched_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
							  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] mismatched_message's realloc is failed.");
				...
				if (privilege_level_id == PRIVILEGE_DB_MANAGER_PRIVILEGE_LEVEL_PLATFORM) {
					...
					size_t new_size = snprintf(0, 0, "%s%s", mismatched_message, message) + 1;
					mismatched_message = realloc(mismatched_message, new_size * sizeof(char));
					TryReturn(mismatched_message, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
							  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] mismatched_message's realloc is failed.");
					strncat(mismatched_message, message, strlen(message));
					ret_val = PRVMGR_ERR_INVALID_PRIVILEGE;
				}
			...
			*error_message = strdup("[INVALID_PARAMETER] Invaild parameter was passed.|");
			TryReturn(error_message != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] error_message's strdup is failed.");
	...
	if (ret_val != PRVMGR_ERR_NONE) {
		...
		TryReturn(message_list != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
				  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] message_list's strdup is failed.");
		if (noexist_message != NULL) {
			...
			TryReturn(message_list != NULL, ret_val = PRVMGR_ERR_OUT_OF_MEMORY;
					  goto FINISH, PRVMGR_ERR_OUT_OF_MEMORY, "[PRVMGR_ERR_OUT_OF_MEMORY] message_list's realloc is failed.");
